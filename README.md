...
)](https://marketplace.visualstudio.com/items?itemName=JohnCole.restclient-mcp) [![Installs](https://vsmarketplacebadges.dev/installs-short/JohnCole.restclient-mcp.svg)](https://marketplace.visualstudio.com/items?itemName=JohnCole.restclient-mcp) [![Rating](https://vsmarketplacebadges.dev/rating-short/JohnCole.restclient-mcp.svg)](https://marketplace.visualstudio.com/items?itemName=JohnCole.restclient-mcp) [![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](LICENSE) [![Bluesky](https://img.shields.io/badge/Bluesky-jwc.dev-0285FF?logo=bluesky&logoColor=white)](https://bsky.app/profile/jwc.dev) [![LinkedIn](https://img.shields.io/badge/LinkedIn-johnwebbcole-0A66C2?logo=linkedin&logoColor=white)](https://www.linkedin.com/in/johnwebbcole)
...
After installing this extension, VS Code discovers the bundled Rest Client MCP server automatically through the extension's MCP server definition provider - no manual step required, and no file gets written to your workspace or user profile for this.
...
3. If your VS Code build/fork doesn't support dynamic MCP server discovery, run `Rest Client MCP: Register MCP Server` to write a `rest-client` entry into your workspace `.vscode/mcp.json` and (when it can reliably locate it) your user profile `mcp.json` instead. Run `Rest Client MCP: Show MCP Server Status` to check whether that fallback is already registered. Don't run this if the server already appears once via dynamic discovery - it will show up as a second, duplicate-looking entry.
...
If your environment blocks automatic (dynamic) discovery, run `Rest Client MCP: Register MCP Server` to write an idempotent `rest-client` entry into workspace/user `mcp.json` on demand instead.
